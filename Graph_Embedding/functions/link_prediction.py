...
def mask_test_edges(adj, test_percent=10., val_percent=5.):
    ...
    assert not np.any(np.isin(idx_test_edges_false, positive_idx))
    assert not np.any(np.isin(idx_val_edges_false, positive_idx))
    assert not np.any(np.isin(val_edges[:, 0] * adj.shape[0] + val_edges[:, 1], train_edges_linear))
    assert not np.any(np.isin(test_edges_linear, train_edges_linear))
    assert not np.any(np.isin(val_edges[:, 0] * adj.shape[0] + val_edges[:, 1], test_edges_linear))
